...
/* ────────────── Inspector: Sequência promo (7 mensagens) ────────────── */

interface PromoMsg { order: number; content: string; enabled: boolean }

function PromoSequenceInspector({ cfg, onUpdate, produtoSlug }: { cfg: any; onUpdate: (p: any) => void; produtoSlug?: string }) {
  const [slug, setSlug] = useState<string>(cfg.produto_slug ?? produtoSlug ?? '');
  const [bucket, setBucket] = useState<'aftersales' | 'cs' | 'spin'>(cfg.bucket ?? 'aftersales');
  const [messages, setMessages] = useState<PromoMsg[]>(cfg.messages ?? []);
  const [productOptions, setProductOptions] = useState<Array<{ slug: string; name: string }>>([]);
  const [loadingProducts, setLoadingProducts] = useState(false);
  const [loadingMessages, setLoadingMessages] = useState(false);
  const [interval, setIntervalSec] = useState<number>(cfg.interval_seconds ?? 86400);

  // Persist whenever local state changes
  useEffect(() => {
    onUpdate({ produto_slug: slug, bucket, messages, interval_seconds: interval });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [slug, bucket, messages, interval]);
    ...
    fetch('https://pgfgripuanuwwolmtknn.supabase.co/functions/v1/knowledge-export-full', {
      ...
      .then((j) => {
        const opts = (j?.products ?? [])
          .map((p: any) => ({ slug: String(p?.slug ?? ''), name: String(p?.name ?? p?.slug ?? '') }))
          .filter((x: any) => x.slug)
  ...
  const fetchMessages = async () => {
    if (!slug) { toast.error('Selecione um produto'); return; }
    ...
    try {
      ...
      const prod = (json?.products ?? []).find((p: any) => p?.slug === slug);
      const raw = prod?.messages?.[bucket] ?? [];
      const mapped: PromoMsg[] = (raw as any[])
        .map((m, i) => ({
      ...
      setMessages(mapped);
      if (mapped.length === 0) toast.warning(`Nenhuma mensagem em ${bucket} para este produto.`);
  ...
  const toggleMsg = (order: number) => {
    setMessages((arr) => arr.map((m) => (m.order === order ? { ...m, enabled: !m.enabled } : m)));
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        <Label className="text-xs">Bucket de mensagens</Label>
      ...
      <div>
        ...
        <Input type="number" value={interval} min={60} onChange={(e) => setIntervalSec(Number(e.target.value))} />
        <p className="text-[10px] text-muted-foreground mt-0.5">86400 = 1 dia · 3600 = 1 hora</p>
      </div>
      <Button variant="outline" size="sm" className="w-full" onClick={fetchMessages} disabled={!slug || loadingMessages}>
      ...
      {messages.length === 0 ? (
        <div className="rounded border border-amber-500/40 bg-amber-500/5 p-2 text-xs text-amber-700 dark:text-amber-400">
          ...
          Se ficar vazio, cadastre a sequência no Sistema A para este produto.
        </div>
      ) : (
