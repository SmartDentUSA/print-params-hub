...
export function PromotionalTablesTab() {
  ...
  return (
    <div className="space-y-5">
      ...
      <Dialog open={Boolean(pickerSection)} onOpenChange={(open) => !open && setPickerSection(null)}><DialogContent className="max-w-3xl"><DialogHeader><DialogTitle>Adicionar produto do catálogo</DialogTitle><DialogDescription>Os dados atuais serão copiados para preservar esta promoção.</DialogDescription></DialogHeader><div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="pl-9" value={catalogSearch} onChange={(e) => setCatalogSearch(e.target.value)} placeholder="Buscar produto, SKU ou apresentação" /></div><div className="max-h-[55vh] space-y-2 overflow-y-auto">{filteredCatalog.map((option) => <button type="button" key={option.key} onClick={() => addCatalogItem(option)} className="flex w-full items-center justify-between gap-3 rounded-md border p-3 text-left hover:bg-muted"><div><p className="font-medium">{option.name}</p><p className="text-xs text-muted-foreground">{[option.variation, option.sku].filter(Boolean).join(" • ") || "Produto"}</p></div><span className="font-semibold">{money(option.price, draft.currency)}</span></button>)}</div></DialogContent></Dialog>
