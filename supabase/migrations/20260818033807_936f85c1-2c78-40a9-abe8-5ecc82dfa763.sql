...
AS $function$
  ...
  keyed AS (
    SELECT t.*,
           CASE
             WHEN length(regexp_replace(COALESCE(t.destino,''), '\D', '', 'g')) >= 10
               THEN right(regexp_replace(t.destino, '\D', '', 'g'), 8)
             ELSE COALESCE(t.lead_id::text, lower(COALESCE(t.destino,'')))
           END AS ckey
    FROM todos t
  ),
  agg AS (
    SELECT k.ckey,
           ...
           (array_agg(k.lead_id ORDER BY (k.lead_id IS NULL), k.sent_at DESC NULLS LAST))[1] AS lead_id,
           ...
           (array_agg(k.nome ORDER BY (k.nome IS NULL), k.sent_at DESC NULLS LAST))[1] AS nome_invite,
