...
BEGIN
  ...
  -- summary por vendedor (aggregate em duas etapas para evitar nested aggregate)
  WITH all_risk AS (
  ...
  per_vendor AS (
    SELECT vendedor,
           count(*) FILTER (WHERE tipo='estagnado')   AS leads_estagnados,
           count(*) FILTER (WHERE tipo='sem_recompra') AS clientes_sem_recompra,
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'vendedor', vendedor,
    'leads_estagnados', leads_estagnados,
    'clientes_sem_recompra', clientes_sem_recompra,
    'total_em_risco', total_em_risco
  ) ORDER BY total_em_risco DESC), '[]'::jsonb)
  INTO v_summary FROM per_vendor;
