...
AS $function$
WITH won_props AS (
  -- One row per deal: the accepted proposal (status Aprovada/Ganha/Aceita/Accepted/Won)
  ...
  WHERE d.status = 'ganha'
    ...
    AND (
      lower(COALESCE(prop->>'status','')) IN ('aprovada','ganha','aceita','accepted','won','approved')
      OR (prop->>'accepted_at') IS NOT NULL
    )
  ORDER BY d.id, COALESCE((prop->>'valor_ps')::numeric, 0) DESC
),
printers AS (
