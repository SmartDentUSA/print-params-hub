...
BEGIN
  WITH ganhos AS (
    ...
    SELECT
      coalesce(sum(CASE WHEN (d.closed_at AT TIME ZONE 'America/Sao_Paulo')::date >= v_ini
                         AND (d.closed_at AT TIME ZONE 'America/Sao_Paulo')::date < v_fim THEN d.value END), 0) AS atual,
      coalesce(sum(CASE WHEN (d.closed_at AT TIME ZONE 'America/Sao_Paulo')::date >= v_prev
                         AND (d.closed_at AT TIME ZONE 'America/Sao_Paulo')::date < v_prev_fim THEN d.value END), 0) AS anterior
    FROM public.deals d
    WHERE coalesce(d.is_deleted, false) = false
