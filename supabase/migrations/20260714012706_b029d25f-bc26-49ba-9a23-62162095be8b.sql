...
WITH deal_edge AS (
  SELECT
    ...
    BOOL_OR(
      (item->>'nome') ILIKE '%Edge Mini%'
      AND (item->>'nome') ~* '(cure|mercury|wash|nano|resina|notebook|ino[[:space:]]|blz[[:space:]]|ryzen|glaze|splint|bite|smartmake|elegoo|kit|vitality|unikk|pod)'
    ) AS is_combo
  FROM deals d
