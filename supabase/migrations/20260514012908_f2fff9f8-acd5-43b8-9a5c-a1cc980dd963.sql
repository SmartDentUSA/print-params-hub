DROP FUNCTION IF EXISTS public.fn_resumo_vendas_mes(integer, integer);
...
AS $function$
  ...
  leads_por_vendedor AS (
    SELECT
      LOWER(TRIM(proprietario_lead_crm)) AS vendedor_norm,
  ...
  SELECT
    ...
    ROUND(b.total_deals::numeric / NULLIF(l.leads_recebidos, 0) * 100, 1) AS taxa_conversao
  FROM base b
  CROSS JOIN total t
