...
BEGIN
  ...
  FOR r IN (
    ...
    v_result := jsonb_set(
      v_result,
      ARRAY[r.workflow_stage, r.workflow_cell],
      jsonb_build_object('label', r.mapped_label, 'layer', 'ativo', 'hits', r.total_hits),
      true
    );
  END LOOP;

  -- ── 2. CONCORRENTE: equip_* fields × workflow_cell_mappings (competitor) ──
  FOR r IN (
    ...
    IF v_result #> ARRAY[r.workflow_stage, r.workflow_cell] IS NULL THEN
      v_result := jsonb_set(
        v_result,
        ARRAY[r.workflow_stage, r.workflow_cell],
        jsonb_build_object('label', r.mapped_label, 'layer', 'conc', 'hits', 1),
        true
      );
    END IF;
  END LOOP;

  -- ── 3. SDR: sdr_*_interesse fields × workflow_cell_mappings (sdr_field) ──
  FOR r IN (
    ...
    IF v_result #> ARRAY[r.workflow_stage, r.workflow_cell] IS NULL THEN
      v_result := jsonb_set(
        v_result,
        ARRAY[r.workflow_stage, r.workflow_cell],
        jsonb_build_object('label', coalesce(r.mapped_label, r.field_value), 'layer', 'sdr', 'hits', 1),
        true
      );
    END IF;
  END LOOP;
...
GRANT EXECUTE ON FUNCTION public.compute_lead_portfolio_from_mappings(uuid) TO authenticated, service_role;
