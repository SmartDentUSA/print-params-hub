-- ============================================================================
-- deals: o upsert não pode regredir para um snapshot mais antigo
--
-- Quando um negócio muda de pipeline no PipeRun mantendo o mesmo deal_id
-- (ganho no Funil de vendas -> CS Onboarding), o sync do pipeline de origem
-- ainda o devolve com o estado anterior. Como o ON CONFLICT DO UPDATE aceitava
-- qualquer snapshot, a MESMA linha de `deals` alternava entre os dois estados a
-- cada rodada de sync — e a receita do Painel Comercial alternava junto.
--
-- Caso observado em 05/08/2026 — deal 62330220 (Lucas Silva):
--   A) Funil de vendas / Fechamento / aberta / R$ 28.000 / piperun_updated_at 03/08 17:25
--   B) CS Onboarding  / Em espera   / ganha  / R$ 24.000 / piperun_updated_at 04/08 11:24
--   piperun_stage_transitions confirma B como estado atual (migrou em 04/08 09:59).
-- Efeito no painel: R$ 24.342,00 entrando e saindo da receita do mês
-- (deal 62330220 R$ 24.000 + deal 62386695 R$ 342).
--
-- Agrava o quadro: o MESMO deal_id aparece no piperun_deals_history de vários
-- leads (1.310 casos, 67 com status conflitante) e 3.877 desses snapshots nem
-- carimbo de data têm. Cada lead reprocessado escreve na mesma linha de `deals`.
--
-- A guarda está no WHERE do ON CONFLICT DO UPDATE, no fim da função: o UPDATE só
-- ocorre se o snapshot que chega tiver carimbo e for igual ou mais recente que o
-- gravado. Snapshot sem carimbo nunca sobrescreve uma linha carimbada.
-- A mesma guarda foi aplicada em upsertDealHistory
-- (supabase/functions/_shared/piperun-field-map.ts), que protege o
-- piperun_deals_history do lead — essa parte só passa a valer no próximo deploy
-- das edge functions.
-- ============================================================================
  ...
  THEN
    ...
    LOOP
      ...
      ON CONFLICT (piperun_deal_id) DO UPDATE SET
        ...
        updated_at = NOW()
      /* anti-regressão: snapshot mais antigo que o já gravado não sobrescreve */
      WHERE deals.piperun_updated_at IS NULL
         OR (EXCLUDED.piperun_updated_at IS NOT NULL
             AND EXCLUDED.piperun_updated_at >= deals.piperun_updated_at);
    END LOOP;
  END IF;
END;
$function$;
