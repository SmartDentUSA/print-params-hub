  ...
  WITH unified AS (
    ...
    SELECT
      ...
      CASE
        ...
        WHEN m.tipo ILIKE '%whats%' OR m.tipo ILIKE '%wa%' OR m.tipo ILIKE '%evolution%' THEN 'WhatsApp enviado'
    ...
    -- 3) Campanhas: SMS/WhatsApp e E-mail (envio, abertura, clique, bounce)
    SELECT * FROM (
      -- 3a) SMS / WhatsApp de campanha (texto completo)
      SELECT
      ...
      UNION ALL
      -- 3b) SMS entregue / falha de entrega (callback do provedor)
      SELECT
    ...
    -- 4) Visitas em paginas / base de conhecimento
    SELECT
      'view:' || v.id::text,
      'conteudo',
      'page_view',
