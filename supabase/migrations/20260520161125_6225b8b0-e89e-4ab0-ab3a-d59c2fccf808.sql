...
AS $$
  ...
  ld AS (
    SELECT f.id AS form_id, count(s.*)::bigint AS leads
    FROM f
    LEFT JOIN lead_form_submissions s
      ON s.form_id = f.id::text
     AND s.submitted_at >= (SELECT since FROM period)
    GROUP BY f.id
  ),
  wins AS (
    SELECT f.id AS form_id, count(DISTINCT la.id)::bigint AS deals_won
    FROM f
    JOIN lead_form_submissions s ON s.form_id = f.id::text
...
GRANT EXECUTE ON FUNCTION public.fn_form_metrics(int) TO anon, authenticated;
