...
SELECT t.id,
    ...
    c.cover_image_url,
    c.signup_form_url,
    COALESCE(( SELECT count(*)::int FROM smartops_enrollment_companions ec JOIN smartops_course_enrollments e ON e.id = ec.enrollment_id WHERE e.turma_id = t.id), 0) AS companions_count
