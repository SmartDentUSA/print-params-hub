...
BEGIN
  ...
  WITH leads AS (
    -- e-mail exato (qualquer coluna de email)
    ...
    SELECT id, nome, email, telefone_normalized, empresa_nome, empresa_cnpj,
           ...
           NULL::text, true
    FROM lia_attendances
    WHERE NOT v_is_email AND merged_into IS NULL AND piperun_id = v_query

    UNION
    -- deal_id existente na tabela deals
    ...
    WHERE NOT v_is_email AND la.merged_into IS NULL
      AND d.piperun_deal_id::text = v_query AND d.is_deleted = false

    UNION
    -- deal_id no histórico
    SELECT la.id, la.nome, la.email, la.telefone_normalized, la.empresa_nome, la.empresa_cnpj,
           la.piperun_id, la.pessoa_piperun_id, la.piperun_deals_history, la.updated_at,
           v_query, false
    FROM lia_attendances la,
      jsonb_array_elements(COALESCE(la.piperun_deals_history,'[]'::jsonb)) dh
    WHERE NOT v_is_email AND la.merged_into IS NULL AND dh->>'deal_id' = v_query

    UNION
    -- telefone / CNPJ / CPF (query numérica com 8+ dígitos)
    SELECT id, nome, email, telefone_normalized, empresa_nome, empresa_cnpj,
           piperun_id, pessoa_piperun_id, piperun_deals_history, updated_at,
           NULL::text, false
    FROM lia_attendances
    WHERE NOT v_is_email AND length(v_digits) >= 8 AND merged_into IS NULL
      AND (
        telefone_normalized LIKE '%' || v_digits
        OR regexp_replace(COALESCE(telefone,''), '\D', '', 'g') LIKE '%' || v_digits
        OR regexp_replace(COALESCE(empresa_cnpj,''), '\D', '', 'g') = v_digits
        OR regexp_replace(COALESCE(pessoa_cpf,''), '\D', '', 'g') = v_digits
      )

    UNION
    -- nome da pessoa / nome da empresa (texto livre, 3+ caracteres)
    SELECT id, nome, email, telefone_normalized, empresa_nome, empresa_cnpj,
           piperun_id, pessoa_piperun_id, piperun_deals_history, updated_at,
           NULL::text, false
    FROM lia_attendances
    WHERE NOT v_is_email AND length(v_digits) < length(v_query) AND length(v_query) >= 3
      AND merged_into IS NULL
      AND (unaccent(lower(COALESCE(nome,''))) LIKE '%' || unaccent(lower(v_query)) || '%'
        OR unaccent(lower(COALESCE(empresa_nome,''))) LIKE '%' || unaccent(lower(v_query)) || '%')
    LIMIT 200
  ),
  deal_rows AS (
    SELECT
...
GRANT EXECUTE ON FUNCTION public.fn_search_deals_for_training(text) TO authenticated, anon, service_role;