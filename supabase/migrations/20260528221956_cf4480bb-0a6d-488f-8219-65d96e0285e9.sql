-- Wrapper público com lock e fallback silencioso
...
DECLARE
  ...
  _age_sec int;
BEGIN
  -- Lock leve para evitar refresh concorrente (id arbitrário 737373)
  ...
  -- Debounce: pula se atualizou há menos de 60s (a menos que forçado)
  ...
  EXCEPTION WHEN OTHERS THEN
    PERFORM pg_advisory_unlock(737373);
    INSERT INTO public.system_health_logs(feature, status, payload)
    VALUES ('copilot_brain_refresh', 'error',
  ...
  INSERT INTO public.system_health_logs(feature, status, payload)
  VALUES ('copilot_brain_refresh', 'ok',
    jsonb_build_object(
...
GRANT EXECUTE ON FUNCTION public.refresh_copilot_brain(boolean) TO authenticated, service_role, anon;

-- Sanity-check: registra diff Cérebro vs SQL ao vivo
...
AS $$
DECLARE
  _brain_receita numeric;
  _live_receita numeric;
  _brain_deals bigint;
  _live_deals bigint;
  _last_refresh timestamptz;
  _diff_pct numeric;
...
BEGIN
  ...
  _result := jsonb_build_object(
    'brain_receita', _brain_receita,
    'live_receita', _live_receita,
    'brain_deals', _brain_deals,
    'live_deals', _live_deals,
    'diff_pct', round(_diff_pct, 2),
    'last_refresh', _last_refresh,
    'age_minutes', EXTRACT(EPOCH FROM (now() - _last_refresh))/60
  );
  ...
  -- Se drift alto, insere alerta crítico no Cérebro
...
GRANT EXECUTE ON FUNCTION public.check_copilot_brain_drift() TO authenticated, service_role;