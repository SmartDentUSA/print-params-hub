-- Helper: somente a impressora Edge Mini (qualquer variação de nome), sem acessórios e sem kits
...
WITH deal_edge AS (
  SELECT
    d.lead_id,
    d.closed_at        AS sale_at,
    d.piperun_deal_id  AS sale_deal_id,
    ...
    BOOL_OR(
      (item->>'nome') ~* '(scanner\s*intraoral|intraoral|medit|itero|trios|primescan|aoralscan|shining|helios|panda\s*p|runyes|launca|freedom|carestream\s*cs\s*3|3shape|emerald|i700|\mINO\s*200\M|kit\s*chairside)'
    )                  AS is_combo
  FROM deals d
...
item_edge AS (
  SELECT
    di.lead_id,
    COALESCE(d.closed_at, di.data_proposta, di.deal_date::timestamptz) AS sale_at,
    d.piperun_deal_id  AS sale_deal_id,
    COALESCE(d.owner_name, di.vendor_name) AS vendor,
    COALESCE(SUM(
      CASE WHEN public.fn_is_rayshape_edge_printer(COALESCE(di.product_name, di.nome_produto))
           THEN COALESCE(di.total_value, di.valor_total, 0) ELSE 0 END
    ), 0)              AS printer_price,
    BOOL_OR(
      COALESCE(di.product_name, di.nome_produto) ~* '(scanner\s*intraoral|intraoral|medit|itero|trios|primescan|aoralscan|shining|helios|panda\s*p|runyes|launca|freedom|carestream\s*cs\s*3|3shape|emerald|i700|\mINO\s*200\M|kit\s*chairside)'
    )                  AS is_combo
  FROM deal_items di
  JOIN deals d ON d.piperun_deal_id = di.deal_id
  WHERE d.status = 'ganha'
    AND (d.is_deleted IS NULL OR d.is_deleted = false)
    AND di.lead_id IS NOT NULL
  GROUP BY di.lead_id, d.closed_at, di.data_proposta, di.deal_date, d.piperun_deal_id, d.owner_name, di.vendor_name
  HAVING BOOL_OR(public.fn_is_rayshape_edge_printer(COALESCE(di.product_name, di.nome_produto)))
),
edge_sales AS (
  SELECT * FROM deal_edge
  UNION ALL
  SELECT * FROM item_edge
),
printers_auto AS (
  SELECT DISTINCT ON (la.id)
    ...
    es.sale_at             AS printer_date,
    es.sale_deal_id        AS printer_deal_id,
    es.vendor              AS vendor,
    es.printer_price       AS printer_price,
    es.is_combo            AS is_combo,
    'auto'::text           AS source
  FROM edge_sales es
  JOIN lia_attendances la ON la.id = es.lead_id
  ...
  ORDER BY la.id, es.sale_at ASC NULLS LAST, es.printer_price DESC
),
combo_by_lead AS (
