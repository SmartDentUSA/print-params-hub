-- Cria fn_owner_purchase_history: histórico cronológico real de deals ganhos de um lead canônico
-- + ciclos reais (diff em dias entre closed_at consecutivos) calculados server-side.
-- Zero alucinação: tudo vem do banco, LLM nunca calcula.
...
DECLARE
  _historico jsonb;
  _ciclos numeric[];
  _ciclo_medio numeric;
  _ciclo_mediano numeric;
  _total_deals int;
  _receita_total numeric;
  _lead_nome text;
  _aviso text;
BEGIN
  -- Sanity: lead canônico
  SELECT nome INTO _lead_nome
  FROM lia_attendances
  WHERE id = _lead_id AND merged_into IS NULL
  LIMIT 1;
  ...
  WITH d AS (
    SELECT
      ...
      (
        SELECT jsonb_agg(jsonb_build_object(
          'produto', di.product_name,
          'qtd', di.quantity,
          'valor', di.total_price
    ...
    ORDER BY COALESCE(d.closed_at, d.piperun_created_at, d.created_at) ASC
  )
  SELECT
    COALESCE(jsonb_agg(jsonb_build_object(
      'deal_id', id,
      'piperun_deal_id', piperun_deal_id,
      'data', data_evento,
      'valor', value,
      'stage', stage_name,
      'itens', COALESCE(itens, '[]'::jsonb)
    ) ORDER BY data_evento), '[]'::jsonb),
    COUNT(*),
    COALESCE(SUM(value), 0)
  INTO _historico, _total_deals, _receita_total
  FROM d;

  -- Ciclos reais (apenas se >= 2 deals)
  IF _total_deals >= 2 THEN
    ...
    INTO _ciclos, _ciclo_medio, _ciclo_mediano
    FROM diffs;
    _aviso := NULL;
  ELSE
  ...
  RETURN jsonb_build_object(
    ...
    '_disclaimer', _aviso,
    'lead_id', _lead_id,
    'lead_nome', _lead_nome,
    'total_deals', _total_deals,
    'receita_total', _receita_total,
...
GRANT EXECUTE ON FUNCTION public.fn_owner_purchase_history(uuid) TO authenticated, service_role;