...
BEGIN
  ...
  -- canônico
  SELECT id, nome, email, telefone_normalized, telefone_raw, cidade, estado,
         especialidade, area_atuacao, empresa_nome, instagram
    INTO v_lead_row
  FROM lia_attendances WHERE id = v_lead;

  -- treinamentos (inscrito ou acompanhante)
  ...
  -- equipamentos comprados (itens de propostas dos negócios do lead)
  ...
  RETURN jsonb_build_object(
    ...
    'cidade', v_lead_row.cidade,
    'estado', v_lead_row.estado,
...
REVOKE ALL ON FUNCTION public.fn_search_testimonial_client(text) FROM public;
GRANT EXECUTE ON FUNCTION public.fn_search_testimonial_client(text) TO authenticated, service_role;