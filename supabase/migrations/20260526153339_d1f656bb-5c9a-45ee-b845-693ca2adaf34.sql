...
DROP VIEW IF EXISTS public.v_relatorio_mes_funil    CASCADE;
...
deals_mes AS (
  SELECT d.* FROM public.deals d, mes
  WHERE COALESCE(d.is_deleted,false)=false
    AND COALESCE(d.pipeline_name,'') NOT IN ('CS Onboarding','Funil Estagnados','Tulip-Teste','Funil Atos','E-book','Exportação')
    AND to_char((d.created_at AT TIME ZONE 'America/Sao_Paulo'),'YYYY-MM')=mes.mes_ref
),
ganhos AS (
  SELECT * FROM deals_mes
  WHERE status='ganha' AND closed_at IS NOT NULL
    AND to_char((closed_at AT TIME ZONE 'America/Sao_Paulo'),'YYYY-MM')=(SELECT mes_ref FROM mes)
)
SELECT
  ...
  (SELECT count(*) FROM deals_mes) AS leads_criados_mes,
...
CREATE VIEW public.v_relatorio_mes_funil
WITH (security_invoker=on) AS
SELECT
...
GRANT SELECT ON public.v_relatorio_mes_funil    TO anon, authenticated;
