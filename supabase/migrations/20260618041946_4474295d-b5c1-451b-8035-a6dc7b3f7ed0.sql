...
SELECT t.id,
    ...
    c.cover_image_url,
    c.signup_form_url,
    COALESCE(( SELECT count(*)::integer FROM smartops_enrollment_companions ec
        JOIN smartops_course_enrollments e ON e.id = ec.enrollment_id
        WHERE e.turma_id = t.id), 0) AS companions_count,
    t.factory_status,
    t.factory_processed_at
FROM smartops_course_turmas t
JOIN smartops_courses c ON c.id = t.course_id;

GRANT SELECT ON public.v_turmas_com_vagas TO anon, authenticated, service_role;