...
CREATE TABLE IF NOT EXISTS public.painel_comercial_cache (
  bloco text NOT NULL,
  mes date NOT NULL,
  payload jsonb NOT NULL,
  updated_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (bloco, mes)
);

GRANT SELECT ON public.painel_comercial_cache TO authenticated;
GRANT ALL ON public.painel_comercial_cache TO service_role;
ALTER TABLE public.painel_comercial_cache ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "painel cache readable by authenticated" ON public.painel_comercial_cache;
CREATE POLICY "painel cache readable by authenticated"
ON public.painel_comercial_cache FOR SELECT TO authenticated USING (true);

-- Refresh all painel blocks for a given month
...
BEGIN
  ...
  -- Funil (tempos calculados só sobre os últimos 90 dias de transições)
  WITH ordem AS (
    SELECT * FROM (VALUES
    ...
    SELECT t.stage_from_name, t.status_after,
    ...
    SELECT stage_from_name,
           ...
           count(*) FILTER (WHERE status_after = 'perdida') perdido
...
-- Read functions now serve the cache
DROP FUNCTION IF EXISTS public.painel_comercial_funil();
DROP FUNCTION IF EXISTS public.painel_comercial_vendedores(date);
DROP FUNCTION IF EXISTS public.painel_comercial_atividades(date);
DROP FUNCTION IF EXISTS public.painel_comercial_origens(date);
DROP FUNCTION IF EXISTS public.painel_comercial_top_produtos(date);

CREATE OR REPLACE FUNCTION public.painel_comercial_bloco(p_bloco text, p_mes date DEFAULT date_trunc('month', now())::date)
RETURNS jsonb
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT coalesce(
    (SELECT payload FROM public.painel_comercial_cache
     WHERE bloco = p_bloco AND mes = date_trunc('month', p_mes)::date),
    '[]'::jsonb)
$$;

CREATE OR REPLACE FUNCTION public.painel_comercial_funil(p_mes date DEFAULT date_trunc('month', now())::date)
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT public.painel_comercial_bloco('funil', p_mes) $$;

CREATE OR REPLACE FUNCTION public.painel_comercial_vendedores(p_mes date DEFAULT date_trunc('month', now())::date)
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT public.painel_comercial_bloco('vendedores', p_mes) $$;

CREATE OR REPLACE FUNCTION public.painel_comercial_atividades(p_mes date DEFAULT date_trunc('month', now())::date)
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT public.painel_comercial_bloco('atividades', p_mes) $$;

CREATE OR REPLACE FUNCTION public.painel_comercial_origens(p_mes date DEFAULT date_trunc('month', now())::date)
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT public.painel_comercial_bloco('origens', p_mes) $$;

CREATE OR REPLACE FUNCTION public.painel_comercial_top_produtos(p_mes date DEFAULT date_trunc('month', now())::date)
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT public.painel_comercial_bloco('top_produtos', p_mes) $$;

CREATE OR REPLACE FUNCTION public.painel_comercial_kpis_cache(p_mes date DEFAULT date_trunc('month', now())::date)
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT coalesce(
    (SELECT payload FROM public.painel_comercial_cache
     WHERE bloco = 'kpis' AND mes = date_trunc('month', p_mes)::date),
    '{}'::jsonb)
$$;

GRANT EXECUTE ON FUNCTION public.painel_comercial_bloco(text, date) TO authenticated;
GRANT EXECUTE ON FUNCTION public.painel_comercial_funil(date) TO authenticated;
...
GRANT EXECUTE ON FUNCTION public.painel_comercial_kpis_cache(date) TO authenticated;
GRANT EXECUTE ON FUNCTION public.painel_comercial_refresh(date) TO service_role;

SELECT cron.unschedule('painel-comercial-refresh')
WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'painel-comercial-refresh');

SELECT cron.schedule(
  'painel-comercial-refresh',
  '*/15 * * * *',
  $$SELECT public.painel_comercial_refresh();$$
);
