    ...
    WHERE
      EXISTS (
        ...
        WHERE lower(coalesce(d->>'status','')) = 'ganha'
          AND coalesce(
                NULLIF(d->>'created_at','')::timestamptz,
                NULLIF(d->>'updated_at','')::timestamptz,
                '1900-01-01'::timestamptz
              ) >= (la.created_at - interval '1 day')
      )
      OR (coalesce(la.ltv_total, 0) > 0)
      OR EXISTS (
        SELECT 1 FROM loja_integrada_orders o
        WHERE o.attendance_id = la.id
          AND coalesce(o.data_pedido, o.created_at) >= (la.created_at - interval '1 day')
      )
    GROUP BY f.id
  ),
  series AS (
