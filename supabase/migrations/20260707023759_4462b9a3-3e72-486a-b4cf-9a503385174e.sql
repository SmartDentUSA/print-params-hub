CREATE OR REPLACE FUNCTION public.fn_smartops_parse_timestamp(p_value text)
RETURNS timestamptz
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  IF v ~ '^\d{2}/\d{2}/\d{4}(\s+\d{2}:\d{2}(:\d{2})?)?$' THEN
    RETURN to_timestamp(v, CASE WHEN v ~ '\s+\d{2}:\d{2}:\d{2}$' THEN 'DD/MM/YYYY HH24:MI:SS' WHEN v ~ '\s+\d{2}:\d{2}$' THEN 'DD/MM/YYYY HH24:MI' ELSE 'DD/MM/YYYY' END);
  END IF;
...
AS $function$
  ...
  pv AS (
    ...
    LEFT JOIN lead_page_views lpv
      ON lpv.page_path IN ('/f/' || f.slug, '/f/lp/' || f.slug)
    ...
    WHERE
      EXISTS (
        ...
        WHERE lower(coalesce(d->>'status','')) = 'ganha'
          AND public.fn_smartops_parse_timestamp(d->>'closed_at') > la.created_at
      )
      OR EXISTS (
  ...
  series AS (
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE lpv.page_path IN ('/f/' || f.slug, '/f/lp/' || f.slug)
        AND lpv.viewed_at >= (SELECT since FROM period)
      GROUP BY 1
    ) s ON TRUE
    GROUP BY f.id
...
GRANT EXECUTE ON FUNCTION public.fn_smartops_parse_timestamp(text) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.fn_form_metrics(integer) TO anon, authenticated, service_role;