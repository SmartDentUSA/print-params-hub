...
WITH deal_edge AS (
  SELECT
    ...
    d.owner_name,
    COALESCE(SUM(
      CASE WHEN (item->>'nome') ILIKE '%Edge Mini%' THEN (item->>'total')::numeric ELSE 0 END
    ), 0) AS printer_price,
    -- Combo: alguma proposta com Edge Mini contém também um scanner intraoral
    BOOL_OR(
      EXISTS (
        SELECT 1 FROM jsonb_array_elements(prop->'items') ii
        WHERE (ii->>'nome') ILIKE '%Edge Mini%'
      )
      AND EXISTS (
        SELECT 1 FROM jsonb_array_elements(prop->'items') ii
        WHERE (ii->>'nome') ~* '(scanner\s*intraoral|intraoral|medit|itero|trios|primescan|aoralscan|shining|helios|panda\s*p|runyes|launca|freedom|carestream\s*cs\s*3|3shape|emerald)'
      )
    ) AS is_combo
  FROM deals d
