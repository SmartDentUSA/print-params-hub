...
CREATE OR REPLACE FUNCTION public.painel_funil_refresh(p_mes date DEFAULT date_trunc('month', now())::date)
RETURNS void
...
BEGIN
  WITH ordem AS (
    ...
    SELECT stage_name,
           count(*) total,
           count(*) FILTER (WHERE status = 'perdida') perdido
    FROM public.deals
    WHERE pipeline_name ILIKE '%vendas%' AND coalesce(is_deleted,false)=false
      AND piperun_created_at >= now() - interval '90 days'
    GROUP BY 1
  ), dur AS (
    SELECT t.stage_from_name,
    ...
    SELECT stage_from_name, count(*) qtd, avg(dias) FILTER (WHERE dias >= 0) dias
    FROM dur WHERE stage_from_name IS NOT NULL GROUP BY 1
  )
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    'etapa', o.etapa, 'ordem', o.ord, 'atual', coalesce(a.n,0),
    'media_dias', round(tm.dias::numeric,1),
    'pct_perda', CASE WHEN coalesce(p.total,0) > 0 THEN round(100.0*p.perdido/p.total,1) END,
    'qtd_saidas', coalesce(tm.qtd,0)
  ) ORDER BY o.ord), '[]'::jsonb) INTO v_payload
  FROM ordem o
  LEFT JOIN abertos a ON a.stage_name = o.etapa
  LEFT JOIN perdas p ON p.stage_name = o.etapa
  LEFT JOIN tempos tm ON tm.stage_from_name = o.etapa;
