...
-- Fix 1: Update buyer_type generated column to use B2B/B2C labels
ALTER TABLE lia_attendances DROP COLUMN buyer_type;
ALTER TABLE lia_attendances ADD COLUMN buyer_type text GENERATED ALWAYS AS (
  CASE
    WHEN empresa_piperun_id IS NOT NULL THEN 'B2B'
    WHEN pessoa_piperun_id IS NOT NULL THEN 'B2C'
    ELSE 'unknown'
  END
) STORED;

-- Fix 2: Update fn_sync_normalized_from_lead to NOT update buyer_type (it's generated)
...
BEGIN
  ...
  IF v_lead.pessoa_piperun_id IS NOT NULL THEN
    INSERT INTO people (
      ...
      v_lead.pessoa_nascimento::date, v_company_id, NOW()
    )
  ...
  -- ─── 3. Update lead FK references (buyer_type is generated, skip it) ───
  ...
  THEN
    ...
    LOOP
      ...
      INSERT INTO deals (
        ...
        (v_deal_element->>'pipeline_id')::int, v_deal_element->>'pipeline_name',
        v_deal_element->>'stage_name', COALESCE(v_deal_element->>'status', 'aberta'),
        (v_deal_element->>'value')::numeric, (v_deal_element->>'value_products')::numeric,
...
BEGIN
  FOR v_lead_id IN 
    SELECT id FROM lia_attendances 
    WHERE merged_into IS NULL
      AND (pessoa_piperun_id IS NOT NULL OR empresa_piperun_id IS NOT NULL)
    ORDER BY created_at DESC
  LOOP
    BEGIN
      PERFORM fn_sync_normalized_from_lead(v_lead_id);
      v_count := v_count + 1;
