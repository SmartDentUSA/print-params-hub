...
DROP FUNCTION IF EXISTS public._debug_pf(uuid);
DROP FUNCTION IF EXISTS public._dbg_inside_fn(uuid);
...
BEGIN
  ...
  FOR r IN (
    ...
    matched AS (
      ...
      JOIN workflow_cell_mappings m
       ...
       AND (
         w.pname = lower(m.mapped_label) OR w.pname = lower(m.mapped_value)
    ...
    -- Ensure stage object exists, then set the cell
    IF v_result -> r.workflow_stage IS NULL THEN
      v_result := v_result || jsonb_build_object(r.workflow_stage, '{}'::jsonb);
    END IF;
    v_result := jsonb_set(v_result, ARRAY[r.workflow_stage, r.workflow_cell],
  ...
  FOR r IN (
    WITH sdr_pairs(stage_hint, field_value) AS (
      VALUES
        ...
        ('etapa_2_cad',           NULLIF(trim(v_lead.sdr_software_cad_interesse), '')),
  ...
  v_result := v_result || jsonb_build_object('summary',
    COALESCE(v_summary, jsonb_build_object('n_ativo',0,'n_conc',0,'n_sdr',0,'n_mapeamento',0)));
