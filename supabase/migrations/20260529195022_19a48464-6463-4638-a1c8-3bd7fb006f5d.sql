...
SELECT g.id AS group_id,
    ...
    ( SELECT count(*) AS count
          ...
          WHERE q.campaign_id = c.id AND q.status = 'sent'::text) AS msgs_sent,
    ( SELECT count(*) AS count
           FROM wa_message_queue q
          WHERE q.campaign_id = c.id AND q.status = 'pending'::text) AS msgs_pending,
    ( SELECT count(*) AS count
           FROM wa_message_queue q
    ...
    (EXISTS ( SELECT 1
          ...
          WHERE cg.group_id = g.id AND cc.group_id IS NULL AND (cc.status = ANY (ARRAY['draft'::text, 'active'::text, 'paused'::text])))) AS in_shared_campaign,
    g.session_health,
    g.consecutive_send_errors,
    g.last_send_error,
    g.last_send_error_at,
    (tm.evolution_group_key_broken_at IS NOT NULL) AS group_key_auto_fallback
   FROM wa_groups g
     LEFT JOIN wa_campaigns c ON c.id = g.active_campaign_id
     LEFT JOIN team_members tm ON tm.evolution_instance_name = g.instance_name;