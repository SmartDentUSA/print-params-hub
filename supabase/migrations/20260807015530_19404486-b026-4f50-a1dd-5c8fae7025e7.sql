...
WITH exact_events AS (
  ...
  FROM exact_events WHERE cid IS NOT NULL GROUP BY lead_id, cid
), fallback_conversions AS (
  SELECT la.id, la.platform_campaign_id::text, COALESCE(la.entrada_sistema, la.created_at)
  ...
  SELECT d.id AS deal_id, d.piperun_deal_id, d.lead_id,
         ...
         d.closed_at, COALESCE(d.piperun_created_at, d.created_at) AS deal_created_at,
  ...
  FROM public.deals d
  JOIN period_conversions c ON c.lead_id = d.lead_id AND c.converted_at <= d.closed_at
  WHERE d.status = 'ganha' AND d.closed_at IS NOT NULL AND COALESCE(d.is_deleted, false) = false
), attributed_won AS (
...
WITH exact_events AS (
  ...
  SELECT lead_id, cid, MAX(converted_at) AS converted_at FROM exact_events WHERE cid IS NOT NULL GROUP BY lead_id, cid
), fallback_conversions AS (
  SELECT la.id, la.platform_campaign_id::text, COALESCE(la.entrada_sistema, la.created_at)
  FROM public.lia_attendances la
  WHERE la.merged_into IS NULL AND la.platform_campaign_id IS NOT NULL
  ...
  SELECT d.*, c.cid, c.converted_at,
         ROW_NUMBER() OVER (PARTITION BY d.id ORDER BY c.converted_at DESC, c.cid) AS attribution_rank
  FROM public.deals d
  JOIN period_conversions c ON c.lead_id = d.lead_id AND c.converted_at <= d.closed_at
  WHERE d.status = 'ganha' AND d.closed_at IS NOT NULL AND COALESCE(d.is_deleted, false) = false
), attributed AS (
  SELECT * FROM won_candidates WHERE attribution_rank = 1 AND cid = p_campaign_id
)
SELECT a.lead_id, la.nome, a.id, a.piperun_deal_id, a.deal_title, a.pipeline_name,
       COALESCE(a.value, a.value_products, 0)::numeric, a.converted_at, a.closed_at,
       journey.deal_created_at, journey.cs_at,
       CASE WHEN journey.cs_at IS NOT NULL AND journey.deal_created_at IS NOT NULL AND journey.cs_at >= journey.deal_created_at
            THEN ROUND((EXTRACT(EPOCH FROM (journey.cs_at - journey.deal_created_at)) / 86400)::numeric, 1) END,
       la.produto_interesse, COALESCE(items.names, ARRAY[]::text[]),
       CASE WHEN la.produto_interesse IS NULL OR items.names IS NULL THEN NULL
            ELSE NOT EXISTS (SELECT 1 FROM unnest(items.names) n WHERE lower(n) LIKE '%' || lower(split_part(la.produto_interesse, ' ', 1)) || '%') END
FROM attributed a
JOIN public.lia_attendances la ON la.id = a.lead_id
LEFT JOIN LATERAL (
  SELECT COALESCE(a.piperun_created_at, a.created_at) AS deal_created_at,
         COALESCE(
           (SELECT MIN(t.transitioned_at) FROM public.piperun_stage_transitions t
            WHERE t.lead_id = a.lead_id AND t.deal_id = a.piperun_deal_id::text
              AND (t.pipeline_name ILIKE '%cs%' OR t.pipeline_name ILIKE '%onboarding%')
              AND t.transitioned_at >= COALESCE(a.piperun_created_at, a.created_at)),
           CASE WHEN a.pipeline_name ILIKE '%cs%' OR a.pipeline_name ILIKE '%onboarding%' THEN a.closed_at END
         ) AS cs_at
) journey ON true
LEFT JOIN LATERAL (
  SELECT array_agg(DISTINCT COALESCE(di.product_name, di.nome_produto)) AS names
  FROM public.deal_items di WHERE di.deal_id = a.piperun_deal_id AND COALESCE(di.product_name, di.nome_produto) IS NOT NULL
) items ON true
