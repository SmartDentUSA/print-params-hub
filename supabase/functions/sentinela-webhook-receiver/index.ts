...
async function handleMessage(instance: string, raw: any) {
  ...
  const remoteJid: string | undefined = key?.remoteJid ?? raw?.remoteJid;
  if (!remoteJid || !remoteJid.endsWith("@g.us")) return { skipped: "not_group" };

  const messageId: string | undefined = key?.id ?? raw?.messageId;
  ...
  const participant: string | undefined = key?.participant ?? raw?.participant;
  ...
  const messageObj = raw?.message ?? raw?.messageStubParameters ?? null;
  const extracted = extractText(messageObj);

  const tsRaw = raw?.messageTimestamp ?? raw?.timestamp ?? null;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Aceita formatos: { event, instance, data } (Evolution v2)
    // ou { event, instance: { instanceName }, data }
    const event: string | undefined = body?.event ?? body?.eventName;
    ...
    if (event && !/messages\.upsert|MESSAGES_UPSERT/i.test(event)) {
      return Response.json({ skipped: "non_message_event", event }, { headers: corsHeaders });
    }

    const payload = body?.data ?? body;
    const messages = Array.isArray(payload?.messages) ? payload.messages : Array.isArray(payload) ? payload : [payload];
    ...
    return Response.json({ ok: true, count: results.length, results }, { headers: corsHeaders });
  } catch (e) {
