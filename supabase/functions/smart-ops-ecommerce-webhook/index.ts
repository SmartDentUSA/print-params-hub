...
function enrichWithOrderHistory(
  ...
  const allOrderSnapshots = orders.map((o) => {
    ...
    return {
      ...
      tracking: oEnvios[0]?.objeto || oEnvios[0]?.url || null,
      url_pagamento: (oPagamentos[0] as Record<string, unknown>)?.link_boleto || (oPagamentos[0] as Record<string, unknown>)?.link_pix || null,
