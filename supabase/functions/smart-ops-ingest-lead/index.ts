...
async function getLiaColumns(
  ...
  try {
    const { data, error } = await supabase.rpc("list_table_columns", { p_table: "lia_attendances" });
    if (!error && Array.isArray(data) && data.length > 0) {
      _liaColumnsCache = new Set((data as Array<{ column_name: string }>).map((r) => r.column_name));
      return _liaColumnsCache;
    }
    console.warn("[ingest-lead] list_table_columns returned no rows:", error);
  } catch (e) {
    console.warn("[ingest-lead] list_table_columns RPC failed:", e);
  }
  // Fail open — we prefer occasional PGRST204 (already logged) over silently
  // dropping every enrichment field.
  return null;
}
  ...
  try {
    ...
    if (existingLead) {
      ...
      try {
        ...
        if (canonPipelineId === 72938 && formName) {
          const effectiveConversionKey =
            conversionKey ||
            (dedupeId ? `meta:redelivery:${String(dedupeId)}` : `form:${formName}:${new Date().toISOString()}`);
          try {
