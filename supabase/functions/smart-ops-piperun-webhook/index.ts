  ...
  try {
    ...
    await supabase.from("lead_activity_log").insert({
      ...
      event_data: {
        ...
        fonte: "piperun",
        // ─── Payload completo do PipeRun (consultável pela timeline/dashboards) ───
        deal_title: dealSnapshot?.deal_title ?? null,
        stage_id: dealSnapshot?.stage_id ?? null,
        last_stage_updated_at: dealSnapshot?.last_stage_updated_at ?? null,
        origin_name: dealSnapshot?.origin_name ?? null,
        origin_group: dealSnapshot?.origin_group ?? null,
        temperature: dealSnapshot?.temperature ?? null,
        probability: dealSnapshot?.probability ?? null,
        loss_reason: dealSnapshot?.loss_reason ?? null,
        loss_comment: dealSnapshot?.loss_comment ?? null,
        forecast_close_at: dealSnapshot?.forecast_close_at ?? null,
        last_contact_at: dealSnapshot?.last_contact_at ?? null,
        value_products: dealSnapshot?.value_products ?? null,
        value_freight: dealSnapshot?.value_freight ?? null,
        freight_type: dealSnapshot?.freight_type ?? null,
        payment_method: dealSnapshot?.payment_method ?? null,
        payment_installments: dealSnapshot?.payment_installments ?? null,
        product: dealSnapshot?.product ?? null,
        items_text: dealSnapshot?.items_text ?? null,
