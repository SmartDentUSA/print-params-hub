...
Deno.serve(async (req) => {
  ...
  async function apiFetch(endpoint: string) {
    ...
    const authParams = `chave_api=${encodeURIComponent(API_KEY)}&chave_aplicacao=${encodeURIComponent(APP_KEY)}`;
    const url = endpoint.includes('?')
      ? `${API_BASE}${endpoint}&${authParams}`
      : `${API_BASE}${endpoint}?${authParams}`;
    ...
    for (; attempt < MAX_RETRIES; attempt++) {
      ...
      if (response.status === 429) {
        ...
        console.warn(`[poll-li] Rate limit → aguardando ${wait}ms (tentativa ${attempt + 1})`);
    ...
    if (attempt === MAX_RETRIES) throw new Error('Máximo de retries excedido');
