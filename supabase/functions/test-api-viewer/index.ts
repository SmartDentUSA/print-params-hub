...
<body>
  <div class="container">
    <header>
      <h1>🚀 API Data Export - Sistema B</h1>
      <p>Página de teste automática sincronizada com o endpoint de exportação de dados</p>
  ...
  <script>
    ...
    async function fetchData() {
      try {
        ...
        console.log('🔄 Buscando dados do endpoint...');
        ...
        if (etag) {
          ...
          console.log('💾 ETag armazenado:', etag);
        }
    ...
    function renderStats(stats) {
      ...
      const statsConfig = [
        { icon: '📦', value: stats.total_brands || 0, label: 'Marcas' },
        { icon: '🖨️', value: stats.total_models || 0, label: 'Modelos' },
        { icon: '⚙️', value: stats.total_parameter_sets || 0, label: 'Parâmetros' },
        { icon: '🧪', value: stats.total_resins || 0, label: 'Resinas' },
        { icon: '📚', value: stats.total_knowledge_contents || 0, label: 'Artigos' },
        { icon: '🎥', value: stats.total_knowledge_videos || 0, label: 'Vídeos' },
        { icon: '🏷️', value: stats.total_keywords || 0, label: 'Keywords' },
        { icon: '👤', value: stats.total_authors || 0, label: 'Autores' }
      ];
    ...
    function renderTabs() {
      const tabs = [
        ...
        { id: 'parameters', label: '⚙️ Parâmetros' },
        { id: 'resins', label: '🧪 Resinas' },
      ...
      content.innerHTML = \`
        ...
        <table>
          ...
          <tbody>
            \${brands.map(brand => \`
              <tr>
                <td>\${brand.image_url ? \`<img src="\${brand.image_url}" class="logo" alt="\${brand.name}">\` : '🖼️'}</td>
    ...
    function renderModels(models) {
      ...
      content.innerHTML = \`
        <h2>🖨️ Modelos de Impressoras (\${models.length})</h2>
        ...
        <table id="models-table">
          ...
          <tbody>
            \${models.map(model => \`
              <tr>
                <td>\${model.image_url ? \`<img src="\${model.image_url}" class="logo" alt="\${model.name}">\` : '🖨️'}</td>
    ...
    function renderParameters(params) {
      ...
      if (!params || params.length === 0) {
        content.innerHTML = '<div class="empty">Nenhum parâmetro encontrado</div>';
        return;
      }

      content.innerHTML = \`
        <h2>⚙️ Conjuntos de Parâmetros (\${params.length})</h2>
        <div class="search-bar">
          <input type="search" class="search-input" placeholder="Buscar parâmetro..." 
          ...
          <tbody>
            \${params.map(param => \`
              <tr>
                ...
                <td>\${param.variant_label || 'Padrão'}</td>
              </tr>
            \`).join('')}
          </tbody>
        </table>
      ...
      content.innerHTML = \`
        ...
        <table id="resins-table">
          ...
          <tbody>
            \${resins.map(resin => \`
              <tr>
                <td>\${resin.image_url ? \`<img src="\${resin.image_url}" class="logo" alt="\${resin.name}">\` : '🧪'}</td>
    ...
    function renderKnowledge(contents) {
      ...
      if (!contents || contents.length === 0) {
        content.innerHTML = '<div class="empty">Nenhum conteúdo encontrado</div>';
        return;
      }

      content.innerHTML = \`
        <h2>📚 Base de Conhecimento (\${contents.length})</h2>
        <div class="search-bar">
          ...
          <tbody>
            \${contents.map(item => \`
              <tr>
                ...
                <td>\${item.videos?.length || 0} vídeos</td>
      ...
      content.innerHTML = \`
        ...
        <table>
          ...
          <tbody>
            \${authors.map(author => \`
              <tr>
                <td>\${author.photo_url ? \`<img src="\${author.photo_url}" class="avatar" alt="\${author.name}">\` : '👤'}</td>
    ...
    function renderKeywords(keywords) {
      ...
      content.innerHTML = \`
        <h2>🏷️ Keywords / External Links (\${keywords.length})</h2>
        <div class="search-bar">
    ...
    async function init() {
      try {
        ...
        document.getElementById('error').innerHTML = \`
          ...
          \${error.message}<br><br>
          Verifique se o endpoint está acessível: <a href="\${API_URL}" target="_blank">\${API_URL}</a>
        \`;
      }
    }
    ...
    setInterval(() => {
      ...
      fetchData().then(data => {
        ...
        console.warn('⚠️ Erro no auto-refresh:', err.message);
      });
    }, 300000); // 5 minutos
  </script>
</body>
...
Deno.serve((req) => {
  ...
  console.log('📄 Servindo interface de teste da API');
