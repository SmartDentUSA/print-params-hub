...
- Fonte: `deals.status='ganha'` JOIN `deal_items` ON `deal_items.deal_id = deals.piperun_deal_id` filtrando `product_name`/`nome_produto` ILIKE `%busca%`.
- Para cada lead canônico (merged_into IS NULL): nome, email, telefone, cidade/uf, data_primeira_compra, data_ultima_compra, qtd_unidades, receita_total, n_deals.
