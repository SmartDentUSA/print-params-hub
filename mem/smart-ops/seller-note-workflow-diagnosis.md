...
**Roteiro com fallback multi-fonte (fix "❓ a descobrir" falso)**: `buildLeadProfilingRoteiro(lead, lookup?)` aceita `RoteiroLookup { formIndex, customFieldsIndex }` — os mesmos índices já montados em `diagnoseLead` (respostas de `smartops_form_field_responses` + `raw_payload.custom_fields`). Precedência por item: (1) coluna direta do `lia_attendances`, (2) `formIndex`, (3) `customFieldsIndex`. Além da chave exata normalizada, cada item tem `aliases` em português (ex.: scanner → `scanner|digitaliza|moldagem digital|captura`) usados como substring match contra as chaves dos índices, porque `field_label` do formulário é pergunta em PT e não nome de coluna. Sem lookup o comportamento antigo é preservado (assinatura retrocompatível).
