---
name: Depoimentos — publicação automática em Story do Instagram, TikTok e YouTube Shorts
description: Após publicar o artigo do depoimento, o cron publica o mesmo vídeo no Story/Reels do IG, TikTok e YouTube Shorts com copy IA, ficha real e CTA Link na Bio
type: feature
---
...
- Canais publicados: Instagram Stories, Instagram Reels, TikTok Vídeo e **YouTube Shorts** (`format: 'Shorts'` → `postType: 'short'`).
- YouTube exige título: enviado em `channels[].title` e `platformSpecificData.title` (`Depoimento: {Nome} — {Curso} | Smart Dent`, máx. 100 chars) + `privacyStatus: 'public'`, `madeForKids: false`. O `content` do Zernio é a descrição.
- `social-publish-worker.buildPlatformSpecificData` promove `ch.title` para `platformSpecificData.title` — não remover.
...
- Publica via `social_scheduled_posts` (status `scheduled`, `publish_now=true`), consumido pelo `social-publish-worker`. Mídia = URL assinada temporária do `training-media-proxy` (TTL 1h), `type: 'video'` explícito.
