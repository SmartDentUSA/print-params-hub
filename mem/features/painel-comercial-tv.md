...
- Receita Stripe (RMS: ativações + mensalidades em `stripe_payment_units`, `ativo <> false`, por `paid_at` em SP) é somada a `receita_mes`, exposta em `receita_stripe` e entra na fatia **software/serviço** (e na base `receita_produtos_total`). Vendas RMS não têm deal no PipeRun, então não há duplicidade com o CRM.
